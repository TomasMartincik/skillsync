...
import { resolveProject, resolveRoot, homeRoot, parseArgs } from './common.js';
...
export async function status(argv, ctx) {
  ...
  if (flags.cached === true) {
    await statusCached(project.dir, manifest);
  } else {
    await statusOnline(project.dir, manifest);
  }
}
 ...
 */
async function statusOnline(root, manifest) {
  ...
  try {
    ...
    for (const skill of names) {
      ...
      await warnHomeScope(root, skill);
    }
  } finally {
 ...
 * @param {import('../manifest.js').Manifest} manifest
 */
async function statusCached(root, manifest) {
  const names = Object.keys(manifest.skills).sort();
 ...
 * Codex reads alongside the project scope without dedup (one cheap lstat). Skipped
 * when the operating root already IS home: there the home copy is the managed one,
 * not a shadow.
 * @param {string} root operating root
 * @param {string} skill
 */
async function warnHomeScope(root, skill) {
  const home = homeRoot();
  if (path.resolve(root) === path.resolve(home)) return; // managing the home scope itself
  const p = path.join(home, '.agents', 'skills', skill);
  try {
    await fs.lstat(p);
